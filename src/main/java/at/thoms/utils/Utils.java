...
public class Utils {
	...
	public static Logger getLogger() {
		if (logger == null) {
			logger = LogManager.getFormatterLogger(SkiMod.MODID);
		}
		return logger;
	}
	...
	public static int calculateRedstone(ItemStackHandler handler) {
		...
		for (int j = 0; j < handler.getSlots(); j++) {
			...
			if (stack != null) {
//				f += (float) stack.stackSize / (float) Math.min(handler.getSlotLimit(j), stack.getMaxStackSize());
				f += (float) stack.stackSize / (float) Math.min(64, stack.getMaxStackSize());
				i++;
			}
		}
	...
	public static boolean isInventoryFull(IItemHandler handler) {
		int filledSlots = 0;
		for (int slot = 0; slot < handler.getSlots(); slot++) {
//			if (handler.getStackInSlot(slot).stackSize == handler.getSlotLimit(slot))
				if(handler.getStackInSlot(slot).stackSize == 64)
